        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
              <Link to="/vesti" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors mb-8">
              ...
              <div className="prose prose-lg max-w-none dark:prose-invert">
                {article.content?.startsWith("<") ? (
                  ...
                  article.content?.split("\n").map((paragraph, i) => {
                    ...
                    return <p key={i} className="text-foreground/80 leading-relaxed mb-5">{trimmed}</p>;
                  })
                )}
              </div>
            </div>
...
function ArticleNotFound() {
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
      ...
      <p className="text-muted-foreground mb-6">Ovaj članak ne postoji ili je uklonjen.</p>
      <Link to="/vesti" className="inline-flex items-center gap-2 text-sm font-semibold text-accent hover:underline">
        <ArrowLeft size={16} /> Nazad na vesti
      </Link>
    </div>
  );
